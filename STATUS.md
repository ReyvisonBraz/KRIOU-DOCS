...
| Auth/RLS manual em produção | 🟢 **execução SEC2.1 concluída (2026-08-17)**: contas A e B confirmadas; 1ª execução parada em `42703` (coluna `phone` inexistente — achado acima); fix mínimo (`id,cpf` sem `phone`) commit `6a5a1f0` local, sem push; gates verdes (571 testes, cobertura 56.77 L, lint, build:ci, scan); rerun `run_id e7d923e3…` com **20/20 cenários `passed`**, `records_cleanup=verified` (limpeza só pelos UUIDs exatos), registros preexistentes preservados. Pendência humana: **remover as contas A e B no Dashboard** (`accounts_cleanup=dashboard_required`) para voltar aos 10 auth users |
