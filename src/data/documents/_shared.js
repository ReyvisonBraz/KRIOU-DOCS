...
/**
 * Gera os campos padrão para uma pessoa física, repetidos para múltiplas partes.
 *
 * Os campos são indexados: {prefix}_0_nome, {prefix}_0_cpf, {prefix}_1_nome, etc.
 * A parte 0 é obrigatória para nome e CPF; as demais são totalmente opcionais.
 *
 * @param {string} prefix - Prefixo dos campos (ex: "vendedor", "comprador")
 * @param {string} papel  - Nome do papel para labels (ex: "Vendedor", "Comprador")
 * @param {number} count  - Quantidade de partes (ex: 3)
 * @returns {Array} - Array de definições de campo
 */
export const pessoaFisicaFieldsRepeated = (prefix, papel, count) => {
  const result = [];
  for (let i = 0; i < count; i++) {
    const num = i + 1;
    const labelSuffix = count > 1 ? ` ${num}` : "";

    result.push(
      field(`${prefix}_${i}_nome`, `Nome Completo do ${papel}${labelSuffix}`, "text", {
        required: i === 0,
        placeholder: "Nome completo como no documento",
        example: "Maria da Silva Santos",
        hint: "Digite o nome completo, exatamente como aparece no RG ou CNH. Não use apelidos.",
        whereFind: "RG, CPF, CNH ou Certidão de Nascimento",
        whyImportant:
          "O nome identifica legalmente a pessoa no contrato. Se estiver diferente do documento oficial, o contrato pode ser contestado.",
        disableable: i !== 0,
      }),
      field(`${prefix}_${i}_cpf`, `CPF do ${papel}${labelSuffix}`, "cpf", {
        required: i === 0,
        placeholder: "000.000.000-00",
        example: "123.456.789-00",
        hint: "O CPF tem 11 dígitos e será formatado automaticamente. É o documento mais importante para identificação.",
        whereFind:
          "Cartão do CPF, CNH, ou consulte gratuitamente no site da Receita Federal (receita.fazenda.gov.br)",
        whyImportant:
      ...
      field(`${prefix}_${i}_rg`, `RG do ${papel}${labelSuffix}`, "text", {
        ...
        hint: "Número do RG seguido do órgão que emitiu (ex: SSP/SP, DETRAN/RJ). Se tiver o novo RG (CIN), pode usar também.",
        whereFind:
          "Documento de identidade (carteira de identidade). O órgão emissor fica na frente do documento, geralmente 'SSP' seguido do estado.",
        whyImportant: "O RG complementa a identificação pelo CPF e torna o contrato mais seguro juridicamente.",
        whatHappensIfEmpty: "O documento será gerado apenas com o CPF. Não prejudica a validade, mas é menos completo.",
        disableable: true,
      }),
      ...
      field(`${prefix}_${i}_estado_civil`, `Estado Civil do ${papel}${labelSuffix}`, "select", {
        ...
        hint: "Selecione o estado civil atual. Em contratos de imóveis, se for casado(a), o cônjuge pode precisar assinar também.",
        whereFind:
          "Certidão de Nascimento (se solteiro) ou Certidão de Casamento (se casado/divorciado/viúvo)",
        whyImportant:
          "Em transações de imóveis e bens de alto valor, o estado civil pode exigir a participação do cônjuge no contrato.",
        whatHappensIfEmpty:
      ...
      field(`${prefix}_${i}_endereco`, `Endereço Completo do ${papel}${labelSuffix}`, "text", {
        ...
        hint: "Endereço residencial com rua, número e bairro. Deve ser o mesmo do comprovante de residência.",
        whereFind:
          "Comprovante de residência: conta de luz, água, telefone ou correspondência bancária (dos últimos 3 meses)",
        whyImportant: "O endereço permite localizar a pessoa e é necessário em caso de notificações judiciais.",
        whatHappensIfEmpty:
...
/**
 * Gera o parágrafo de qualificação para múltiplas partes, unidas por " — E — ".
 *
 * Exemplo de saída para 2 vendedores:
 *   VENDEDORES: {vendedor_0_nome}... — E — {vendedor_1_nome}...
 *
 * @param {string} prefix - Prefixo dos campos (ex: "vendedor")
 * @param {string} rotulo - Rótulo no documento (ex: "VENDEDORES")
 * @param {number} count  - Quantidade de partes (ex: 3)
 * @returns {string} - Template do parágrafo com placeholders
 */
export const parteQualificacaoTextRepeated = (prefix, rotulo, count) => {
  const parts = [];
  for (let i = 0; i < count; i++) {
    parts.push(
      `{${prefix}_${i}_nome}{?, , {${prefix}_${i}_nacionalidade}}{?, , {${prefix}_${i}_estado_civil}}{?, , de profissão {${prefix}_${i}_profissao}}{?, portador(a) do RG n.º {${prefix}_${i}_rg} e }inscrito(a) no CPF sob n.º {${prefix}_${i}_cpf}{?any, , residente e domiciliado(a) em {${prefix}_${i}_endereco?}, {${prefix}_${i}_cidade?}}`
    );
  }
  return `${rotulo}: ${parts.join(" \u2014 E \u2014 ")}`;
};
